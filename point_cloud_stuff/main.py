...
dset = GeoDataset('/home/slee/stuff/terrapixel/terraslam/data/dc/dc.tiff')

with pylas.open('data/USGS_LPC_MD_VA_Sandy_NCR_2014_18SUJ321308_LAS_2015.laz') as fh:
    ...
    #color = np.hstack( (color,inten[:,np.newaxis]) )
    ...
    pts = pts * 5000
    pts = pts - pts.mean(0)
    print('mean/std',pts.mean(0), pts.std(0))
...
class LasApp(SingletonApp):
    def __init__(self, wh, cloud, color=None):
        ...
        self.eye = np.zeros(3)
        self.eyeOff = np.array((-20,-20,100.))
    ...
    def updateCamera(self, dt):
        speed = np.linalg.norm(self.eye+self.eyeOff-self.center0) / 100 + .01
        scroll_speed = (speed) * 3 + .1
